...
export default function VentasPage() {
  ...
  // Full refresh cada 30s como fallback (SSE maneja actualizaciones en tiempo real)
  useEffect(() => {
  ...
  useSSE({
    ...
    onSnapshot: useCallback((msg: { todaySalesTotal?: number; todaySalesCount?: number }) => {
      setSalesStats((prev) => ({
        ...prev,
        todayTotalUSD: (msg.todaySalesTotal as number) ?? prev.todayTotalUSD,
        todaySalesCount: (msg.todaySalesCount as number) ?? prev.todaySalesCount,
      }));
    }, []),
  });
  ...
  const exportSalesPDF = async () => {
    ...
    doc.text("Reporte de Ventas — Sakura Studio", 14, 15);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text(`Generado: ${new Date().toLocaleDateString("es-MX")} | Filtro: ${currencyFilter} | Buscar: ${search || "—"}`, 14, 22);
    ...
    const rows = sales.map((sale) => [
      ...
      sale.totalBs ? `Bs ${sale.totalBs.toFixed(2)}` : "—",
    ]);
    const foot = [["", "", "", "", "TOTAL", `$${sales.reduce((s, v) => s + v.total, 0).toFixed(2)}`, salesStats.monthlyTotalBs > 0 ? `Bs ${salesStats.monthlyTotalBs.toFixed(2)}` : "—"]];

    autoTable(doc, {
      startY: 28,
      head: [["Fecha", "Cliente", "Atendió", "Servicios", "Pago", "USD", "Bs"]],
      body: rows,
      foot,
      theme: "grid",
  ...
  const exportSalesExcel = () => {
    ...
    const rows = [
      ["Fecha", "Cliente", "Atendió", "Servicios", "Pago", "USD", "Bs"],
      ...sales.map((sale) => [
      ...
      [],
      ["", "", "", "", "TOTAL", sales.reduce((s, v) => s + v.total, 0), salesStats.monthlyTotalBs],
    ];
    const ws = XLSX.utils.aoa_to_sheet(rows);
    ws["!cols"] = [{ wch: 14 }, { wch: 20 }, { wch: 18 }, { wch: 30 }, { wch: 16 }, { wch: 12 }, { wch: 14 }];
    XLSX.utils.book_append_sheet(wb, ws, "Ventas");
    XLSX.writeFile(wb, `ventas-${new Date().toISOString().split("T")[0]}.xlsx`);
  };
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border border-border bg-white text-muted hover:bg-surface hover:text-dark hover:border-primary/40 transition-all duration-200"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border border-border bg-white text-muted hover:bg-surface hover:text-dark hover:border-emerald-400/40 transition-all duration-200"
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark transition-all duration-200 shadow-sm"
              >
